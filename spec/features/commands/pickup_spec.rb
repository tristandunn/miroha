...
describe "Sending the pickup command", :js do
  ...
  it "broadcasts the pickup action to other characters in the room" do
  ...
  it "removes the item from surroundings for other characters in the room" do
    using_session(:nearby_character) do
      sign_in_as_character create(:character, room: room)
    end
  ...
  context "with partial item name match" do
    it "picks up the item using partial name" do
      item = create(:item, owner: room, name: "golden sword")
  ...
  context "with unknown item name" do
    ...
    it "displays the invalid item error message to the sender" do
