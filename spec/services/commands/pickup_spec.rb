...
describe Commands::Pickup, type: :service do
  let(:character) { create(:character, room: item.owner) }
  let(:instance)  { described_class.new("/pickup #{item.name}", character: character) }
  ...
  describe "#call" do
    ...
    context "with partial item name match" do
      let(:instance) { described_class.new("/pickup #{item.name.slice(0, 2)}", character: character) }
      let(:result)   { instance_double(described_class::Success) }
    ...
    context "with case insensitive item name" do
      let(:instance) { described_class.new("/pickup #{item.name.upcase}", character: character) }
      let(:result)   { instance_double(described_class::Success) }
    ...
    context "with multiple matching items" do
      ...
      before do
        create(:item, :room, owner: item.owner, name: item.name)
      ...
      it "delegates to success handler for first matching item" do
        call
    ...
    context "with item in different room" do
    ...
    context "with invalid item name" do
      let(:instance) { described_class.new("/pickup Unknown", character: character) }
      let(:result)   { instance_double(described_class::InvalidItem) }
    ...
    context "with blank item name" do
      let(:instance) { described_class.new("/pickup ", character: character) }
