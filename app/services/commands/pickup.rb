...
module Commands
  class Pickup < Base
    ...
    #
    # @return [Success] If the item was successfully picked up.
    def success
      Success.new(character: character, item: item)
    end

    # Validate a name parameter is present and the item exists in the room.
    #
    # @return [MissingItem] If the name parameter is blank.
    # @return [InvalidItem] If the item is not found in the room.
